...
type Scene =
  ...
  | "live"
  | "chat"
  // "focus" / "graph" / "return" removed : orphaned
  // beats from the old scripted-demo iteration. Live now advances
  // straight to dashboard. handleBookSlot also goes to dashboard.
  | "dashboard"
  | "roadmap"
  // "rhythm" removed — roadmap now transitions
  // directly to thesis.
  | "thesis"
  | "wordmark";
...
// (ChatTurn / FRESH_SCRIPT / RETURN_SCRIPT removed
// orphaned with the focus / graph / return scenes. The current chat
// surface is InteractiveChatScene, which uses a different turn type
// for the real /api/chat handshake.)
...
function ColosseumDemoInner() {
  ...
  // (turns / typing state removed — they backed the
  // scripted ChatLayout, which went away with the focus/graph/return
  // scenes. memoryNodes is still used by the dashboard's identity
  // graph card.)
  ...
  // the spec calls for the personalized "Ania remembers me" moment.
  ...
  // Restore wallet name hover-identification after the CSS icon-only
  ...
  useEffect(() => {
    ...
    // Deep-link sessions: if we landed on a scene that needs an auth
    // cookie (live needs /api/session; chat hits /api/chat which works
    // anonymously but is friendlier with a context cookie), silently
    // mint a guest session so the scene actually works. Without this,
    // ?scene=live deep-links 401 on /api/session and fall through to
    // chat with no Anja personality cookie. Skip when landing on the
    // entry scenes — those run handleStart themselves.
    if (sceneParam && sceneParam !== "landing" && sceneParam !== "consent") {
      fetch("/api/demo-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: u || "guest" }),
      })
        .then((r) => {
  ...
  // Per-scene auto-advance watchdog (product note).
  //
  // Most of the linear-demo handlers schedule "this scene auto-advances
  // to the next one after Xs" timers inside the handler that *entered*
  // the scene — handleStart queues securing→secured→avatar, handleReturn
  // queues return→dashboard, handleRoadmapNext queues thesis→wordmark.
  // That's fine in the normal flow, but it means jumping to one of those
  // scenes via the SceneJumper (Shift+P) or a ?scene=… deep-link leaves
  // the user frozen on a frame that's supposed to be transient.
  //
  // This effect watches the scene state and queues the right auto-
  // advance regardless of how the scene was entered, so the presenter
  // can rehearse from any beat and so judges deep-linking land somewhere
  // that progresses on its own. Tracked timeouts so jumping mid-advance
  // cancels cleanly.
  useEffect(() => {
    // Plain setTimeout (not setTrackedTimeout) — the effect's own cleanup
    // handles cancellation on scene change. Going through the tracked-
    // timeouts set would mean a SceneJumper jump (clearTrackedTimeouts)
    // also wipes the watchdog timer, which is the inverse of what we
    // want: when the presenter jumps to securing, the watchdog should
    // re-schedule the advance, not get cleared by the jump.
    let id: ReturnType<typeof setTimeout> | undefined;
    if (scene === "securing") {
      id = setTimeout(() => setScene("secured"), 3000);
    } else if (scene === "secured") {
      id = setTimeout(() => setScene("avatar"), 3500);
    } else if (scene === "thesis") {
      id = setTimeout(() => setScene("wordmark"), 3600);
    }
    // (return-scene branch removed — scene retired.)
    return () => {
  ...
  function handleStart() {
    ...
    // securing → secured → avatar advances are owned by the scene
    // watchdog useEffect (single source of truth, search "scene-watchdog
    // useEffect" in this file). The watchdog fires on scene entry
    // regardless of how the scene was entered (handler / SceneJumper /
    // deep-link), so handleStart no longer schedules its own timers
    // here — that would race with the watchdog and any future timing
    // tweak in one place would flicker the other. The the configured 3s
    // securing dwell + the May audit's 6.5s-to-avatar are both encoded
    // in the watchdog (3000 + 3500 = 6500). code review .
  }
  ...
  function handleBookSlot(slot: string) {
    setBookedSlot(slot);
    // : graph scene removed; booking now proceeds
    // directly to the on-chain dashboard. memoryNodes is set anyway
    // for the dashboard's identity-graph card.
    setMemoryNodes(["Booked", "Identity", "Pitch"]);
    setScene("dashboard");
  }
  function handleLiveEnd() {
    // : graph scene removed; live → dashboard direct.
    setMemoryNodes(["Pitch", "Identity", "Focus"]);
    setScene("dashboard");
  }

  // (runFreshScript / handleStartFocus / handleReturnUser removed
  // audit — they backed the focus/graph/return scenes,
  // all of which are gone. The chat surface is now
  // InteractiveChatScene, which talks to /api/chat directly.)

  // --- DASHBOARD -> ROADMAP -> THESIS -> WORDMARK -----------------------
  function handleClose() {
    setScene("roadmap");
  }
  // rhythm scene removed; roadmap now goes
  // straight to thesis, then on to the wordmark close-out.
  // (thesis → wordmark advance is owned by the scene watchdog.)
  ...
  const scenes: Scene[] = [
    ...
    "live",
    "chat",
    // "focus" / "graph" / "return" removed (audit)
    "dashboard",
    "roadmap",
    // "rhythm" removed
    "thesis",
 ...
 rather than templated UI.*/}
            ...
            {/* the design: heading reads without the trailing
 ...
 25% have a wallet → connect; 75% don't → app generates one.*/}
        ...
        {scene === "consent" && (
          <Frame key="consent" maxWidth={520}>
            {/* House style: drop the trailing period to keep the heading
                visually quieter — matches the "one beat / one breath"
                pacing of the other section headings. */}
        ...
        {scene === "securing" && (
          <Frame key="securing" maxWidth={520} center>
            ...
            {/* House style: no period after the subcopy. */}
        ...
        {scene === "avatar" && (
          ...
          >
            ...
            {/* the design & 13: the label reads "COMPANIONS"
 (not "THE CAST") to keep the language warm. The numeral and
 dashed line stay the same chapter device.*/}
            {/* : renumbered to sequential walk order
 (was 03). Linear flow now reads 01→02→03→04→05→06.*/}
            <div style={styles.editorialCredit}>
 ...
 unselected dim. Hover reveals the bio without committing.*/}
              <ol style={styles.castList}>
                {AVATARS.map((a, i) => {
                  ...
                  // (19:34): only Anja is interactive in
              ...
              <div data-cast-feature style={styles.castFeature}>
                <AnimatePresence mode="wait">
                  {avatar ? (
                    ...
                    >
                      {/* the design: phrasing flips from
 ...
 form, more like a question to a friend.*/}
        ...
        {scene === "chat" && (
          <InteractiveChatScene
            key="chat-interactive"
            avatar={avatar ?? "Anja"}
            wallet={wallet}
            identityHash={identityHash}
            onSeeOnChain={() => setScene("dashboard")}
          />
        )}

        {/* (audit): focus / graph / return scenes
 removed. The orphaned ChatLayout, runFreshScript, and
 "simulate return user" right-rail button went with them.
 Live now advances directly to dashboard.*/}
 ...
 flagged as the root "AI-template" tell.*/}
            <div data-dash-header style={styles.dashHeader}>
              <div>
                {/* : renumbered 02→05 so the linear
 walk reads sequentially (avatar=02, memories=03,
 availability=04, dashboard=05, roadmap=06). The
 old "02 Your COLONII" was design slot;
 the demo's experiential order is what matters
 for a judge walking it cold.*/}
                <div style={styles.editorialCredit}>
                  <span style={styles.editorialNumeral}>05</span>
                  <span style={styles.editorialDots} />
                  <span style={styles.editorialLabel}>Your COLONII</span>
                </div>
                {/* the design: heading becomes the three-word
            ...
            <div data-dash-split style={styles.dashSplit}>
              <div style={styles.dashColPrimary}>
                ...
                <ol style={styles.timelineEditorial}>
                  {/* the design: timeline copy rewritten to
 ...
 wordmark in the asset bundle. Natural aspect
 is 7024×4100 (~1.71); pre-allocating proportional space to
 prevent CLS while the image decodes.*/}
              {/* no `priority` — this scene only renders at the end of
 ...
 avatar, identity, memories) starts fresh.*/}
        ...
        {/* Demo status pill (surface demoLoggedIn)*/}
...
/* ── Interactive chat scene ────────────────────────────────────
 Real, working chat with Anja backed by /api/chat streaming.
 This replaces the orphaned scripted "chat" scene from earlier
 iterations of the demo (runFreshScript existed but was never
 called by the live flow, so jumping to ?scene=chat landed on
 an empty surface with a hardcoded disabled input,
 evening: "the chat still doesn't work, in the chat
 page".)

 Mirrors LiveScene's chat tier (same /api/chat handshake, same
 __greeting__ trigger, same streaming protocol) but skips the
 Tavus/Daily video plumbing because this scene is text-only by
 design. /api/chat is already serving clean "I'm Anja, from a
 place called COLONII…" copy in under 1.3s on prod, so this
 surface inherits all of that without bot-server dependency.
 ─────────────────────────────────────────────────────────────*/
type ChatMsg = { role: "user" | "assistant"; content: string };

function InteractiveChatScene({
  avatar,
  wallet,
  identityHash,
  onSeeOnChain,
  ...
  // Auto-scroll on new turns or streaming-bubble appearance.
  useEffect(() => {
    if (scrollRef.current) {
      scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
    }
  }, [chat.length, streamingContent]);

  // Greet on mount.
  //
  // mobile Safari user reported an empty thread on
  // /colosseum?scene=chat — between InteractiveChatScene mounting
  // and the first byte of /api/chat coming back, there's a window
  // where the user sees nothing (the streaming bubble only renders
  // when `streamingContent !== null`). On a slow / backgrounded
  // mobile connection that window can be multi-seconds and 's
  // screenshot caught it.
  //
  // Fix: set streamingContent to an empty string IMMEDIATELY (before
  // the fetch even starts) so the "…" thinking bubble appears the
  // instant the scene mounts. Stream fills it; on commit we move
  // the accumulated text into `chat` and clear streamingContent.
  // If the fetch fails or hangs, the catch / 15s AbortController
  // path drops a scripted greeting into `chat` so the user always
  // sees a message.
  useEffect(() => {
    const SCRIPTED_GREETING =
      "Hey, I'm Anja. I live in COLONII — a quiet place full of forests and lakes. What's your name?";

    let cancelled = false;
    const controller = new AbortController();
    const safety = window.setTimeout(() => controller.abort(), 15_000);

    // Immediate "thinking" bubble — visible at first paint, before fetch.
    setSending(true);
    setStreamingContent("");
      ...
      try {
        ...
        if (!cancelled) {
          setChat([
            {
              role: "assistant",
              content: acc.trim() ? acc : SCRIPTED_GREETING,
            },
          ]);
          setStreamingContent(null);
        }
      } catch {
        // Network error, abort, or parse failure — always leave the user
        // with a visible message rather than an empty thread.
        if (!cancelled) {
          setChat([{ role: "assistant", content: SCRIPTED_GREETING }]);
          setStreamingContent(null);
        }
  ...
  // Component-scoped AbortController for in-flight sends, so unmounting
  // the chat scene mid-stream (SceneJumper, back button) cleanly aborts
  // the reader and skips post-unmount setState calls.
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      const t = text.trim();
      if (!t || sending) return;
      // Build history that INCLUDES the user's just-typed turn, then
      // commit it to UI state. Without this we send n-1 turns to the
      // LLM (it sees the new message via `message:` but not in the
      // history array), degrading multi-turn coherence (code review
      // ).
      const history = [...chat, { role: "user", content: t } as ChatMsg];
      setChat(history);
      setDraft("");
      setSending(true);
      ...
      try {
        ...
        if (!res.ok || !res.body) {
          setChat((prev) => [
            ...prev,
            {
              role: "assistant",
              content:
                "Hmm — I'm having trouble reaching the words right now. Try again?",
            },
          ]);
          return;
        }
        ...
        setChat((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "Hmm — something dropped on the way through. Want to try that again?",
          },
        ]);
        setStreamingContent(null);
      } finally {
    ...
    >
      <div style={styles.chatLeft}>
        ...
        <div style={styles.composer}>
          <div style={styles.composerInner}>
            <label htmlFor="colosseum-chat-scene-input" className="sr-only">
              Message to {avatar}
            </label>
            <input
              id="colosseum-chat-scene-input"
              autoFocus
              type="text"
              value={draft}
              ...
              placeholder={`Type your message to ${avatar}…`}
      ...
      <aside style={styles.chatRight}>
        ...
        <p style={styles.faintSm}>
          Anything worth remembering becomes a tiny verifiable record. You never
        ...
        {/* chat scene was a dead-end with no way
 forward. Surface a CTA into the dashboard ("on-chain
 manifestation") so the judge can advance from the chat
 tier into the rest of the demo.*/}
        {onSeeOnChain && (
          <button
            type="button"
            onClick={onSeeOnChain}
            style={{ ...styles.ctaSmall, marginTop: 18, width: "100%" }}
            aria-label="See your COLONII on-chain"
          >
            See it on-chain →
          </button>
        )}
...
const FALLBACK_MEMORIES: Record<string, string[]> = {
  ...
  guest: [],
  // Seeded internal-demo accounts. Memory text is intentionally empty
  // in the public build so no real personal info ships with the repo.
  // The Supabase `demo_users` table (separate, server-side only) is
  // the source of truth when an internal presenter logs in.
  sam: [],
  aileen: [],
  luke: [],
  lollie: [],
          ...
          >
            ...
            {data?.identity && (
              <>
                ...
                {/* dropped the "Mock-mode SDK" phrasing.
...
function MemoriesFrame({
  ...
  return (
    ...
    >
      {/* : renumbered 04→03 (sequential walk).*/}
      <div style={styles.editorialCredit}>
...
function AvailabilityFrame({
  ...
  return (
    <motion.section
      ...
      style={{ ...styles.frame, maxWidth: 720, zIndex: 1 }}
    >
      {/* : renumbered 05→04 (sequential walk).*/}
      <div style={styles.editorialCredit}>
        <span style={styles.editorialNumeral}>04</span>
        <span style={styles.editorialDots} />
        <span style={styles.editorialLabel}>{avatarName}'s availability</span>
      </div>
      {/* Warmer header: instead of "Anja is selective with her time"
          we use "Anja is looking forward to hanging out with you!".
          Exclamation, no period — house style still applies.
          avatarName stays parametrised. */}
...
// RhythmFrame removed entirely from the demo flow.
...
function RoadmapFrame({ onNext }: { onNext: () => void }) {
  return (
    ...
    >
      ...
      {/* Roadmap layout: chapter device + heading + 3 cohort columns
          + CTA. The numerical scaling story is covered by the
          voice-over rather than an in-frame strip. */}
...
const AVATARS: {
  ...
  // if the file 404s. Assets live in /public/personas/.
  portrait?: string;
}[] = [
  {
    name: "Anja",
...
const styles: Record<string, React.CSSProperties> = {
  ...
  // Chapter device — fully green line across each section indicator:
  // numeral, dashed bar, and label all tinted to the neon accent
  // (#0EF283). The all-green treatment makes the divider read as
  // design signage that anchors every chapter rather than a utility
  // separator.
  editorialCredit: {
    display: "flex",
    alignItems: "center",
  ...
  editorialDots: {
    ...
    // the long-dash treatment in the brand system.
    backgroundImage:
  ...
  // editorialCredit inside dashHeader now carries its own borderBottom
  // divider — removing borderBottom + paddingBottom here prevents the
  // double horizontal rule on the dashboard frame.
  dashHeader: {
    display: "grid",
  ...
  // A calm body-font subline sits directly under the three-beat
  // headline. Keeps the page breathing instead of jamming the chain
  ...
  // ElevenLabs partner credit — persistent corner mark on the
  // back-half scenes of the demo. The asset is black-on-transparent
  // (the "Grants" mark); we invert it via CSS so it renders white
  // on the pure-black page background.
