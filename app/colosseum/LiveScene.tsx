...
// Sam confirmed (2026-05-11 19:55) the chat tier should be kept and
// fully wired with Ania's Colony lore + AH-niah pronunciation; the
...
// Set true when the initial-connect effect enters; cleared on cleanup.
// If a second mount fires before the first resolves, it bails immediately.
let connectingLock = false;

// MED-10: throttle memory anchors to max one per 10 seconds.
let lastAnchorAt = 0;
...
export default function LiveScene({
  ...
  // MED-13: separate streaming state so existing bubbles don't reconcile per token
  ...
  // --- start the session --------------------------------------------------
  useEffect(() => {
    // H2: bail if another mount is already connecting
  ...
  // Identifiers are deterministic — same input → same hash → same PDA.
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      if (!text.trim()) return;
      // MED-9: capture user message without stale chat snapshot —
      // append user turn functionally, then stream assistant into streamingContent.
      setChat((prev) => [...prev, { role: "user", content: text }]);
      setDraft("");
      setSending(true);

      // MED-10: Substantive turns get a real on-chain memory anchor, throttled
      ...
      try {
        ...
        while (true) {
          ...
          if (done) break;
          acc += decoder.decode(value, { stream: true });
          // MED-13: update streaming bubble only — existing bubbles don't reconcile
          setStreamingContent(acc);
        }
  ...
  // version fired per token during streaming and got janky (audit M5).
  useEffect(() => {
  ...
  function handleEnd() {
    // Audit L5: confirm before ending a real Daily.co session — billing stops
      ...
      <div style={S.left}>
        <header style={S.header}>
          <div style={S.headerLeft}>
            {/* Aileen's slides 18/20/22: a small COLONII mark anchors
        ...
        <div style={S.stage}>
          ...
          {/* (Sam 2026-05-11 19:55: the "unavailable" surface from a
            ...
            {anchors.map((a) => (
              ...
              >
                <div style={S.anchorTop}>
                  ...
                  {/* MED-11: RelTime owns its own interval — no outer setNow tick */}
                  <RelTime at={a.at} />
                </div>
...
// MED-11: owns its own 5s interval so the outer LiveScene doesn't re-render
    ...
    <div style={S.chatStage}>
      <div style={S.chatThread}>
        <div style={S.chatThreadInner}>
          ...
          {/* MED-13: streaming bubble is separate — existing bubbles don't reconcile per token */}
...
// MED-12: META hoisted to module scope — not recreated per render.
...
// Aileen's slides 18/20/22 paint the right-side mode indicator in a
...
const S: Record<string, any> = {
  ...
  // Aileen's slides 18/20/22 anchor the call surface with a small
  ...
  // HIGH-4: when mode is "video" but callState is still "idle" (Daily
  ...
  dot: (s: CallState, mode: Mode = "video"): React.CSSProperties => {
    ...
    // HIGH-4: video mode but no frame yet → stay blue (loading)
