...
export default function AvatarView({ roomUrl, token, connecting, onStateChange, onError, onUserRequestChat }: AvatarViewProps) {
  ...
  //
  // code review: same ref-mirror applied to
  // onUserRequestChat — LiveScene passes a fresh arrow fn each render
  // so the 10s escape timer kept restarting whenever the parent
  // re-rendered (callState ticks, anchor pulses). The effect now
  // depends on a stable boolean instead.
  const onStateChangeRef = useRef(onStateChange);
  const onErrorRef = useRef(onError);
  const onUserRequestChatRef = useRef(onUserRequestChat);
  useEffect(() => {
    onStateChangeRef.current = onStateChange;
    onErrorRef.current = onError;
    onUserRequestChatRef.current = onUserRequestChat;
  });
  const hasChatEscape = !!onUserRequestChat;
  ...
  // dead button. Depends on `hasChatEscape` (stable boolean), not the
  // raw function identity, so parent re-renders don't reset the timer.
  useEffect(() => {
    if (connected || !hasChatEscape) {
      setShowChatEscape(false);
      return;
    }
    const t = setTimeout(() => setShowChatEscape(true), 10_000);
    return () => clearTimeout(t);
  }, [connected, hasChatEscape]);
    ...
    <div className="avatar-container" style={{ background: showContainer ? "#000" : "transparent", pointerEvents: showContainer ? "auto" : "none" }}>
      ...
      {!connected && (roomUrl || connecting) && (
        <div className="connecting-overlay">
          ...
          {showChatEscape && hasChatEscape && (
            <button
              type="button"
              onClick={() => onUserRequestChatRef.current?.()}
