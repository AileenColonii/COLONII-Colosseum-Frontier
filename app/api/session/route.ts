    ...
    if (mode === "video" && TAVUS_API_KEY) {
      ...
      const tavusRes = await fetch("https://tavusapi.com/v2/conversations", {
        ...
        body: JSON.stringify({
          ...
          // memories. Note the spelling — "Anya" forces the TTS engine to
          // produce the two-syllable "AHN-yah" sound. The earlier phonetic
          // "Ania" was being read as three syllables "AH-nee-ah" by
          // ElevenLabs in English voice; "Anja" reads as the harsh
          // English "AHN-ja" — neither is what we want.
          custom_greeting:
            userContext && userContext.displayName && userContext.displayName !== "Guest"
              ? `Hey ${userContext.displayName}, good to see you again.`
              : "Hey, I'm Anya. What's your name?",
          properties: {
