...
function getAuthSecret(): string {
  ...
  // Prefer a dedicated session secret; accept BOT_SERVER_SECRET for
  // backward compat with deploys that haven't rotated yet. Never fall
  // back to SUPABASE_SERVICE_ROLE_KEY — using the DB master key as the
  // HMAC key means rotating one rotates the other, and an incidental
  // leak of the service-role key would let an attacker forge session
  // tokens for any user. (Security audit .)
  const secret = process.env.SESSION_SECRET || process.env.BOT_SERVER_SECRET;
  if (!secret) {
    throw new Error("Missing auth secret: set SESSION_SECRET (or BOT_SERVER_SECRET)");
  }
  if (secret === process.env.SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error(
      "SESSION_SECRET must not equal SUPABASE_SERVICE_ROLE_KEY — use a dedicated value"
    );
  }
  cachedSecret = secret;
  return cachedSecret;
